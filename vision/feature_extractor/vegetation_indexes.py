...
def gemi(nir, red, **kwargs):
    ...
    gemi_val = eta * (1 - 0.25 * eta) - num_deno_nan_divide((red - 0.125), (1 - red), return_numpy=True)
...
def tdvi(nir, red, **kwargs):
    ...
    denominator = np.sqrt(nir ** 2 + red + 0.5)
...
def ndre(nir, red, **kwargs):
    red_edge = num_deno_nan_divide(nir, red, return_numpy=True) - 1
...
def pvi(nir, red, a=0.3, b=0.5, **kwargs):
    ...
    return (nir - a * red - b) / np.sqrt(1 + a**2)
