...
def calc_frame_physical_parmas(xyz_point_cloud, binary_box_img, ndvi_binary):
    ...
    frame_physical_parmas["perimeter"] = calc_tree_perimeter(xyz_point_cloud,
                                                             ndvi_binary)  # np array (scaler for each row)
...
def create_3d_fruit_space(tracker_results, tree_images, tree_name):
    ...
    for i, frame_number in enumerate(tree_images.keys()):
        if i < n_frames/4:
            continue
        print(f"fruit space - {tree_name}: {frame_number}")
...
def update_tree_images_traker_results(tree_images, tracker_results, shapes_x, shapes_y,
                                      tree_folder, frame, zed_shift, all_coords, tracker_full_results):
    ...
    if not (os.path.exists(fsi_path) and os.path.exists(zed_path)):
        return tree_images, tracker_results, shapes_x, shapes_y, False
    pictures_dict = {}
    ...
    if np.isfinite(cur_coords["tx"][0]):
        ...
        return tree_images, tracker_results, shapes_x, shapes_y, False
    imgs_shape = pictures_dict["zed"].shape
    shapes_x.append(imgs_shape[1])
    shapes_y.append(imgs_shape[0])
    tree_images[frame] = pictures_dict
    tracker_results[frame] = {row[1]["track_id"]: row[1]["bbox"] for row in
                                   tracker_full_results[tracker_full_results["image_id"] == frame][
                                       ["track_id", "bbox"]].iterrows()}
    return tree_images, tracker_results, shapes_x, shapes_y, True


def get_slicer_data(slice_path, max_w):
...
def create_row_features(path_to_row, zed_shift=0, max_x=600, max_y=900, save_csv=True, block_name="", max_z=0):
    ...
    for tree in trees[1:-1]: # change back to [1:-1]
        ...
        tree_images, slicer_results, tracker_results = preprocess_tree(tree_folder, tracker_path,
                                                                       slice_path, zed_shift=zed_shift,
    ...
    if save_csv:
        df.to_csv(os.path.join(path_to_row, "row_features.csv"))
    return df
