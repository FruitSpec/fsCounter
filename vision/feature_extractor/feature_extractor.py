...
from vision.misc.help_func import update_save_log
...
def transform_to_vi_features(features_dict):
    ...
    for key in vegetation_indexes_keys:
        ...
        features_dict[key] = np.nanmean(clean_key)
...
def init_fruit_params(value):
    ...
    :return: an initialized features dictionary
    """
    features_list = ["cv", "cv2", "cv3", "cv4", "cv5", "frame", "w_h_ratio", "q1", "q3",
                     "avg_intens_arr", "med_intens_arr"]
    return fill_dict(features_list, value)
...
def calc_frame_physical_parmas(xyz_point_cloud, binary_box_img, ndvi_binary):
    ...
    pixel_size_x, pixel_size_y, pixel_are = get_real_world_dims_with_correction(xyz_point_cloud[:, :, 2])
    total_orange = np.nansum(binary_box_img * pixel_are)
    total_foliage = np.nansum(ndvi_binary * pixel_are)
...
def calc_physical_features(tree_images, slicer_results, minimal_frames, tracker_results, tree_name, masks=None, dets_only=False):
    ...
    if len(minimal_frames) > 1 and isinstance(masks, type(None)):
        try:
            ...
            masks = np.ones_like(tree_images[list(tree_images.keys())[0]]["fsi"])
    ...
    for i, frame_number in enumerate(minimal_frames):
        ...
        if i > 0:
            ...
            if isinstance(mask, type(None)):
                print("skipped mask: ", i)
                continue
            false_mask = mask.astype(bool)
        ...
        # debugg
        # plot_2_imgs(ndvi_binary, xyz_point_cloud, title=f"{frame_number}_fe",
        #             save_to = f"/media/fruitspec-lab/easystore/debugging/feature_extractor_class_comparison/old/physical_params_imgs/image_slice_{frame_number}.png")
        # plt.hist(frame_physical_parmas["width"], bins=50)
        # plt.vlines(np.nanmean(frame_physical_parmas["width"]), 0, 100, color="black")
        # plt.title(f"width distribution, {frame_number}_fe")
        # plt.savefig(f"/media/fruitspec-lab/easystore/debugging/feature_extractor_class_comparison/old/physical_params_imgs/hist_{frame_number}.png")
        #
        # plt.show()
...
def tree_intensity_summary(tree_fruit_params, tree_name, frame_number):
    ...
    :param tree_fruit_params: dictionary containing intensity for each fruit
...
def calc_vi(tree_images, slicer_results, minimal_frames, tree_name, masks=None):
    ...
    for i, frame_number in enumerate(minimal_frames):
        ...
        if i > 0:
            false_mask = masks[i - 1]
            if isinstance(false_mask, type(None)):
                false_mask = np.ones_like(ndvi_binary)
            false_mask = false_mask.astype(bool)
            ndvi_binary[false_mask] = np.nan
...
def calc_fruit_features(tree_images, tracker_results, tree_name):
    ...
    for i in range(2, 6):
        tree_fruit_params[f"cv{i}"] = tracker_results[f"cv{i}"]
...
def init_3d_fruit_space(tracker_results, tree_images):
    ...
    """
    all_frames = [frame for frame in tracker_results.keys() if frame not in ["cv"] + [f"cv{i}" for i in range(2, 6)]]
...
def create_3d_fruit_space(tracker_results, tree_images, tree_name, max_depth_change=0):
    """
    ...
    last_frame_boxes = fruit_3d_space
    for i, frame_number in enumerate(tree_images.keys()):
        if i < n_frames/4:
            continue
        ...
        new_boxes, old_boxes, boxes_w_frame, boxes_h_frame = get_new_old_boxes(xyz_point_cloud,
                                                                               last_frame_boxes, boxes, nir, swir_975)
        boxes_w = np.append(boxes_w, boxes_w_frame)
        boxes_h = np.append(boxes_h, boxes_h_frame)
        # 20%
        fruit_3d_space, shift = project_boxes_to_fruit_space_global(fruit_3d_space,
                                                                         last_frame_boxes, old_boxes,
                                                                         new_boxes, max_depth_change,
                                                                         shift)
        last_frame_boxes = {**new_boxes, **old_boxes}
...
def filter_tracker_results(tracker_results, slicer_results, tree_images, max_z):
    ...
    uniq, counts = np.unique(
        ...
        return_counts=True)
    tracker_results["cv"] = len(uniq)
    for i in range(2, 6):
        tracker_results[f"cv{i}"] = len(uniq[counts >= i])
    # res = []
    # for frame_id in tracker_results.keys():
    #     frame_tracks = tracker_results[frame_id]
    #     for track_id, track in frame_tracks.items():
    #         res.append([frame_id, track_id, track[0][0], track[0][1], track[1][0], track[1][1]])
    # pd.DataFrame(res, columns=["frame_id", "track_id", "x1", "y1", "x2", "y2"]).to_csv(
    #     "/media/fruitspec-lab/easystore/debugging/feature_extractor_class_comparison/old.csv")
    return tracker_results

def save_imgs(tree_imgs, masks):
    for i, frame_number in enumerate(tree_imgs.keys()):
        ndvi_binary = tree_imgs[frame_number]["ndvi_binary"].astype(np.uint8).copy()*255
        binary_box_img = tree_imgs[frame_number]["binary_box_img"].astype(np.uint8).copy()*255
        fsi = tree_imgs[frame_number]["fsi"].astype(np.uint8).copy()
        zed_rgb = tree_imgs[frame_number]["zed_rgb"].astype(np.uint8).copy()
        if i > 0:
            mask = masks[i - 1]
            if isinstance(mask, type(None)):
                print("skipped mask: ", i)
                continue
            false_mask = mask.astype(bool)
            ndvi_binary[false_mask] = 0
            binary_box_img[false_mask] = 0
        else:
            false_mask = np.zeros_like(ndvi_binary.shape)

        m_path = "/media/fruitspec-lab/easystore/debugging/feature_extractor_class_comparison/old"
        cv2.imwrite(os.path.join(m_path, "ndvi_binary", f"frame_{frame_number}.png"),
                    ndvi_binary)
        cv2.imwrite(os.path.join(m_path, "binary_box", f"frame_{frame_number}.png"),
                    binary_box_img)
        cv2.imwrite(os.path.join(m_path, "false_mask", f"frame_{frame_number}.png"), false_mask.astype(np.uint8)*255)
        cv2.imwrite(os.path.join(m_path, "fsi", f"frame_{frame_number}.png"), fsi.astype(np.uint8)[:,:,::-1])
        cv2.imwrite(os.path.join(m_path, "rgb_zed", f"frame_{frame_number}.png"), zed_rgb.astype(np.uint8)[:,:,::-1])
...
def extract_features_for_tree(tree_images, slicer_results, tracker_results, tree_name, max_z=0):
    ...
    # minimal_frames = minimal_frames[int(n_mininal*0.25): int(n_mininal*0.75):1]
...
def update_tree_images_tracker_results(tree_images, tracker_results, tree_folder, frame, zed_shift, all_coords,
    ...
    if cur_coords.shape[0] == 0 or np.any(np.isnan(cur_coords[["tx", "ty", "sx", "sy"]])):
        return tree_images, tracker_results, False
...
def read_preprocess_results(tracker_path, tree_id=-1):
    ...
    if "frame" in tracker_full_results:
        tracker_full_results.rename({"frame": "frame_id"}, axis=1, inplace=True)
...
def resize_dicts(tree_images,slicer_results, tracker_results, slice_path, frame,max_y,max_x,max_z,tracker_full_results,
    ...
    tree_images[frame]["zed"] = remove_high_blues(tree_images[frame]["zed"], tree_images[frame]["zed_rgb"][:, :, 2])
    tree_images[frame]["zed"][tree_images[frame]["zed_rgb"][:, :, 0] >
                              tree_images[frame]["zed_rgb"][:, :, 1] + 10] = np.nan
    if max_z > 0:
...
def preprocess_debug(keys, tree_images, tracker_results, slicer_results, max_y, prefix=""):
    ...
    for frame in keys:
        ...
        # plt.imshow(zed)
        # plt.show()
        saving_folder = os.path.join("/media/fruitspec-lab/easystore/testing_playground", prefix)
...
def preprocess_tree_fe_pipe(row_path, tree_id, zed_shift=0, max_x=None,
    ...
    for i, frame in enumerate(jai_frames):
        ...
        try:
            ...
            if frame not in jai_zed.keys():
                print(" no key loading problem: ", frame)
            else:
                print("loading problem: ", frame)
            ret = False
        if not ret:
...
def create_row_features_fe_pipe(path_to_row, zed_shift=0, max_x=600, max_y=900, save_csv=True, block_name="", max_z=0,
    ...
    for tree_id in tqdm(trees):
        ...
        percent_seen = load_json(os.path.join(path_to_row, "percent_seen.json"))
        tree_features["percent_seen"] = np.mean([percent_seen[int(key)]
                                         for key in tree_images.keys() if int(key) in list(percent_seen.keys())])
        tree_features["block_name"] = block_name
...
if __name__ == '__main__':
    ...
    row_paths = ["/media/fruitspec-lab/cam175/APPLECHILE04_test/290323/APPCALIB/R5A"]
    norgb = False
    for row_path in row_paths:
        ...
        create_row_features_fe_pipe(row_path, zed_shift=zed_shift, max_x=600, max_y=900,
                                    save_csv=True, block_name=block_name, max_z=max_z, debug=False, save_name="test.csv",
