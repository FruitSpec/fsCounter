...
def last_nonzero(arr, axis, invalid_val=-1):
    ...
    vec = np.where(mask.any(axis=axis), val, invalid_val)
    vec = [v for v in vec if v != -1]
    return vec

def first_nonzero(arr, axis, invalid_val=-1):
    mask = arr != 0
    vec = np.where(mask.any(axis=axis), mask.argmax(axis=axis), invalid_val)
    vec = [v for v in vec if v != -1]
    return vec
...
def is_gap(cropped_depth, vec, signal_thrs=20, percentile=0.85):
    if len(vec) < 2:
        return vec

    height = cropped_depth.shape[0]
    width = vec[1] - vec[0]

    search_area = cropped_depth[:, vec[0]: vec[1]].copy()
    score = np.sum(search_area < signal_thrs) / (height * width)
    if score >= percentile:
        vec = [(vec[0] + vec[1]) // 2]
