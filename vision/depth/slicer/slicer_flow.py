...
def slice_clip(filepath, output_path, rotate=0, window_thrs=0.4, neighbours_thrs=250, dist_thrs=150, signal_thrs=0.5,start_frame=0, end_frame=None, smooth=True, debug=False):
    ...
    #max_workers = 4
    #cams = [video_wrapper(filepath, rotate=rotate, depth_minimum=depth_minimum, depth_maximum=depth_maximum) for _ in range(max_workers)]
    while True:
        pbar.update(1)
        #pbar.update(4)
        ...
        #results, new_index = slice_multithread(cams, index)
        #for res in results:
        #    if res is not None:
...
def slice_multithread(cams, index, max_workers=4):
    max_index = cams[0].get_number_of_frames()
    step = max_workers
    if index + step > max_index:
        step = max_index - index
    cam_list = [cams[i] for i in range(step)]
    index_list = [index + i for i in range(step)]

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        results = list(executor.map(worker, cam_list, index_list))

    return results, index + step




def post_process(slice_data):
...
def is_wide_gap(depth, start=750, end=1250, signal_thrs=20, percentile=0.75):
    width = depth.shape[1]
...
if __name__ == "__main__":
    #file_path = "/home/yotam/FruitSpec/Sandbox/slicer_test/caracara_R2_3011/ZED_1.svo"
    #file_path = "/home/yotam/FruitSpec/Data/DWDB_Jan_2023/DWDBCL46/R11/ZED_1.svo"
    file_path = "/home/yotam/FruitSpec/Data/DWDB_Jan_2023/DWDBCN51/R21/ZED_1.svo"
    #depth_folder_path = "/home/yotam/FruitSpec/Sandbox/slicer_test/caracara_R2_3011/sliced3/depth"
    #rgb_folder_path = "/home/yotam/FruitSpec/Sandbox/slicer_test/caracara_R2_3011/sliced3/frames"
    output_path = "/home/yotam/FruitSpec/Sandbox/DWDB_2023/DWDBCN51/R21_auto_full"
    neighbours_thrs = 350
    signal_thrs=0.3
    window_thrs = 0.6
    dist_thrs = 300
    ...
    #                  end_frame=None,
    #                  debug=debug)
    #write_json({"filepath": file_path, "output_path": output_path, "data": data})

    #slicer_validator(file_path, output_path,data=None, rotate=2)
    slice_data = load_json(file_path, output_path)
    post_process(slice_data=slice_data)
