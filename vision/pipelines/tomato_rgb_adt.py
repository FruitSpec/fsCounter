...
def run(cfg, args, metadata=None, n_frames=None):
    ...
    while f_id < n_frames:
        ...
        _, _, jai_batch, rgb_batch = adt.get_frames(f_id)
        ...
        tomato_colors = list(map(get_trks_colors, rgb_batch, trk_outputs))
        tomato_colors_viz = list(map(tomato_color_class_to_rgb, tomato_colors))
        # collect results:
        results_collector.collect_tracks(trk_outputs)
        results_collector.debug_batch(f_id, args, trk_outputs, det_outputs, rgb_batch, None, trk_windows,
                                      tomato_colors_viz)
...
def get_trks_colors(rgb_img, trk_outputs):
    colors = []
